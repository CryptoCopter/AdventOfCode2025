...
const LEFT: Direction = Direction{d_row: 0, d_column: -1};
const RIGHT: Direction = Direction{d_row: 0, d_column: 1};
const UP: Direction = Direction{d_row: 1, d_column: 0};
const DOWN: Direction = Direction{d_row: -1, d_column: 0};
    ...
    P: AsRef<Path>,
{
    ...
    for (row_index, line) in io::BufReader::new(file).lines().map(|l| l.unwrap()).enumerate() {
        let mut row: Vec<bool> = Vec::new();
        for (column_index, char) in line.chars().enumerate() {
            match char {
                '@' => {
                    row.push(true);
                    rolls.push(Location{row: row_index, column: column_index});
                },
                _ => row.push(false),
            }
        }
...
fn solve(input: ParsedInput) -> (u32, u32) {
    ...
    let part_2: u32 = 0;
    let (floorplan, rolls) = input;
    let around: [Direction; 8] = [LEFT, LEFT+UP, UP, UP+RIGHT, RIGHT, RIGHT+DOWN, DOWN, DOWN+LEFT];
